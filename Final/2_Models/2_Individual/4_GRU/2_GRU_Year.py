...
for i in tqdm(range(0, len(test_set))):
    ...
    #Test
    model=GRU(input_size,hidden_size,output_size,n_layers)
...
#GRU Evaluation
# =========================
GRU_RMSE = metrics.mean_squared_error(y_actual, y_pred_list, squared = False)
GRU_MAE = metrics.mean_absolute_error(y_actual, y_pred_list)

print("GRU RMSE: " + str(GRU_RMSE))
print("GRU MAE: " + str(GRU_MAE))
...
plt.title('GRU - First Five Days').set_fontsize(20)
plt.plot(y_actual[:240],'--', label="Actual Value", color='#8c8488')
plt.plot(y_pred_list[:240], label="GRU", color='#0a4236')
