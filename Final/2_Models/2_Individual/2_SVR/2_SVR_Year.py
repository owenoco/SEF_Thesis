...
for i in tqdm(range(0, len(test_set))):
    ...
    SVR_test = pickle.load(open('SVR_pickle', 'rb'))
    
    #train and predict 
    SVR_test.fit(X_f_train, y_f_train)
    test_pred = SVR_test.predict(X_test)
...
#RFR Evaluation
# =========================
SVR_RMSE = metrics.mean_squared_error(y_test_list, y_test_pred, squared = False)
SVR_MAE = metrics.mean_absolute_error(y_test_list, y_test_pred)

print("SVR RMSE: " + str(SVR_RMSE))
print("SVR MAE: " + str(SVR_MAE))
...
plt.title('SVR Model - First Five Days').set_fontsize(20)
plt.plot(y_test_list[:240],'--', label="Actual Value", color='#8c8488')
plt.plot(y_test_pred[:240], label="SVR", color='#0a4236')
